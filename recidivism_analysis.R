...
boxplot(data$decile_score.1 ~ data$age_cat, main="Boxplot of Decile Score by Race", xlab="Race", ylab="Decile Score")
boxplot(dataset$decile_score.1 ~ dataset$age_cat, main="Boxplot of Decile Score by Race", xlab="Race", ylab="Decile Score")

dataset_101 <- read.csv('C:\\Users\\91838\\Downloads\\Boston University\\CS699 - Data Mining\\abcde.csv')

# Remove NA values from the data
dataset_101 <- na.omit(dataset_101)

ggplot(dataset_101, aes(x = decile_score, fill = sex)) +
  geom_bar(position = "dodge") +
  labs(title = "Decile Score by Sex", x = "Decile Score", y = "Count")

ggplot(dataset_101, aes(x = decile_score, fill = age_cat)) +
  geom_bar(position = "dodge") +
  labs(title = "Decile Score by Age Category", x = "Decile Score", y = "Count")

ggplot(dataset_101, aes(x = decile_score, fill = race)) +
  geom_bar(position = "dodge") +
  labs(title = "Decile Score by Race", x = "Decile Score", y = "Count")

ggplot(dataset_101, aes(x = v_score_text, fill = sex)) +
  geom_bar(position = "dodge") +
  labs(title = "Score text by Sex", x = "Score Text", y = "Count")

ggplot(dataset_101, aes(x = v_score_text, fill = age_cat)) +
  geom_bar(position = "dodge") +
  labs(title = "Score Text by Age Category", x = "Score Text", y = "Count")

ggplot(dataset_101, aes(x = v_score_text, fill = race)) +
  geom_bar(position = "dodge") +
  labs(title = "Score Text by Race", x = "Score Text", y = "Count")



library(dplyr)
df_summary <- dataset_101 %>% 
  group_by(race) %>% 
  summarize(count = n())

ggplot(data = df_summary, aes(x = race, y = count)) +
  geom_bar(stat = "identity")

ggplot(data = dataset_101, aes(x = age_cat, y = priors_count)) +
  geom_bar()
